...
class TablaCorteOperaciones extends SampleView {
  @override
  _TablaCorteOperacionesState createState() => _TablaCorteOperacionesState();
}
...
class _TablaCorteOperacionesState extends SampleViewState {
  CorteOperacionesDataGridSource datoSource = CorteOperacionesDataGridSource();
  final ColumnSizer _columnSizer = ColumnSizer();
  GridLinesVisibility gridLineVisibility;
  CustomPopupMenu _selectedChoices;
  CorteOperaciones eci = CorteOperaciones();
  ...
  void initState() {
    ...
    corteOperaciones = Sesion.miSesion.corteOperaciones;
  }
            ...
            child: Row(
              ...
              children: [
                Container(
                  ...
                  child: Text(
                      'Registros totales - ${Sesion.miSesion.corteOperaciones.length}'),
                ),
                Container(
                  child: Row(
                    children: [
...
/*------------------------------------------------------------------------*/
  Widget getCellWidget(BuildContext context, GridColumn column, int rowIndex) {
    if (eci.titulos[column.mappingName] == "Texto")
      return formateaTexto(eci.obtenValor(
          column.mappingName, Sesion.miSesion.corteOperaciones[rowIndex]));
  ...
  obtenTitulos() {
    ...
    CorteOperaciones().titulos.forEach((key, value) {
      if (value == 'Texto')
        encabezados.add(
          GridTextColumn(
            mappingName: key,
...
class CorteOperacionesDataGridSource extends DataGridSource<CorteOperaciones> {
  CorteOperacionesDataGridSource();
  CorteOperaciones eci = CorteOperaciones();
  @override
  List<CorteOperaciones> get dataSource => corteOperaciones;
  @override
  Object getValue(CorteOperaciones _datos, String columName) {
