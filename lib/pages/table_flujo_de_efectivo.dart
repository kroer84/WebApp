...
class TablaFlujoDeEfectivo extends SampleView {
  @override
  _TablaFlujoDeEfectivoState createState() => _TablaFlujoDeEfectivoState();
}

List<FlujoDeEfectivo> corteFlujoDeEfectivo;
...
class _TablaFlujoDeEfectivoState extends SampleViewState {
  FlujoDeEfectivoDataGridSource datoSource = FlujoDeEfectivoDataGridSource();
  final ColumnSizer _columnSizer = ColumnSizer();
  GridLinesVisibility gridLineVisibility;
  CustomPopupMenu _selectedChoices;
  FlujoDeEfectivo eci = FlujoDeEfectivo();
  ...
  void initState() {
    ...
    corteFlujoDeEfectivo = Sesion.miSesion.edoFlujoDeEfectivo;
  }
            ...
            child: Row(
              ...
              children: [
                Container(
                  ...
                  child: Text(
                      'Registros totales - ${Sesion.miSesion.edoFlujoDeEfectivo.length}'),
                ),
                Container(
                  child: Row(
                    children: [
...
/*------------------------------------------------------------------------*/
  Widget getCellWidget(BuildContext context, GridColumn column, int rowIndex) {
    if (eci.titulos[column.mappingName] == "Texto")
      return formateaTexto(eci.obtenValor(
          column.mappingName, Sesion.miSesion.edoFlujoDeEfectivo[rowIndex]));
    else if (eci.titulos[column.mappingName] == "Entero")
      return formateaEntero(eci.obtenValor(
          column.mappingName, Sesion.miSesion.edoFlujoDeEfectivo[rowIndex]));
    else if (eci.titulos[column.mappingName] == "lista")
      return formateaTexto(eci.obtenValor(
          column.mappingName, Sesion.miSesion.edoFlujoDeEfectivo[rowIndex]));
  ...
  obtenTitulos() {
    ...
    FlujoDeEfectivo().titulos.forEach((key, value) {
      if (value == 'Texto')
        encabezados.add(
          GridTextColumn(
            mappingName: key,
...
class FlujoDeEfectivoDataGridSource extends DataGridSource<FlujoDeEfectivo> {
  FlujoDeEfectivoDataGridSource();
  FlujoDeEfectivo eci = FlujoDeEfectivo();
  @override
  List<FlujoDeEfectivo> get dataSource => corteFlujoDeEfectivo;
  @override
  Object getValue(FlujoDeEfectivo _datos, String columName) {
